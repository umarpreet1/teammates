...
function addRankOption(questionNum) {
    ...
    $('<div id="rankOptionRow-' + curNumberOfChoiceCreated + '-' + questionNum + '">' + '<div class="input-group">' + ('<input type="text" name="' + FEEDBACK_QUESTION_RANKOPTION + '-' + curNumberOfChoiceCreated + '" ') + ('id="' + FEEDBACK_QUESTION_RANKOPTION + '-' + curNumberOfChoiceCreated + '-' + questionNum + '" ') + 'class="form-control rankOptionTextBox">' + '<span class="input-group-btn">' + '<button class="btn btn-default removeOptionLink" id="rankRemoveOptionLink" ' + ('onclick="removeRankOption(' + curNumberOfChoiceCreated + ',' + questionNum + ')" tabindex="-1">') + '<span class="glyphicon glyphicon-remove"></span>' + '</button>' + '</span>' + '</div>' + '</div>').insertBefore($('#rankAddOptionRow-' + questionNum));
