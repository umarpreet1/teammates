...
function addMsqOption(questionNum) {
    ...
    $('<div id="msqOptionRow-' + curNumberOfChoiceCreated + '-' + questionNum + '">' + '<div class="input-group">' + '<span class="input-group-addon">' + '<input type="checkbox" disabled>' + '</span>' + ('<input type="text" name="' + FEEDBACK_QUESTION_MSQCHOICE + '-' + curNumberOfChoiceCreated + '" ') + ('id="' + FEEDBACK_QUESTION_MSQCHOICE + '-' + curNumberOfChoiceCreated + '-' + questionNum + '" ') + 'class="form-control msqOptionTextBox">' + '<span class="input-group-btn">' + '<button type="button" class="btn btn-default removeOptionLink" id="msqRemoveOptionLink" ' + ('onclick="removeMsqOption(' + curNumberOfChoiceCreated + ',' + questionNum + ')" tabindex="-1">') + '<span class="glyphicon glyphicon-remove"></span>' + '</button>' + '</span>' + '</div>' + '</div>').insertBefore($('#msqAddOptionRow-' + questionNum));
